...
class Fetcher:
    ...
    async def full_fetch(self):
        ...
        if self.stations_file.exists():
            ...
            logger.error(f"站点文件不存在: {self.stations_file} 或 {self.location_file}")
            raise FileNotFoundError(f"站点文件不存在: {self.stations_file} 或 {self.location_file}")
        ...
        try:
            ...
            for site_name, stats in site_stats.items():
                logger.info(f"站点 {site_name} - 总数: {stats['site_total']}, 可用: {stats['site_available']}, 已用: {stats['site_used']}, 错误: {stats['site_error']}")
                print(f"Site {site_name} - Total: {stats['site_total']}, Available: {stats['site_available']}, Used: {stats['site_used']}, Error: {stats['site_error']}")
                site_info = {
                    "site_name": site_name,
