...
class ProviderManager:
    ...
    async def fetch_all_providers(self) -> Dict[str, Any]:
        ...
        for idx, (provider, result) in enumerate(zip(provider_list, fetch_results)):
            if isinstance(result, Exception):
                logger.error(f"服务商 {provider.provider_name} 获取数据失败: {result}", exc_info=True)
    ...
    async def fetch_and_format(self, provider_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """获取数据并格式化为 API 响应格式
