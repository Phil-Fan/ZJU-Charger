...
def pattern_scan(ids_generator: Generator[str, None, None], max_workers: int = 50) -> List[Tuple[str, str, int, int, int]]:
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_address = {executor.submit(get_device_info, address): address for address in all_ids}
...
# --- 核心聚合逻辑 ---
def aggregate_results(results: List[Tuple[str, str, int, int, int]]) -> List[Dict[str, Any]]:
    """
...
# --- CSV 输出函数（针对聚合格式） ---
def write_to_csv(aggregated_results: List[Dict[str, Any]], output_filename: str = "aggregated_device_results.csv"):
    """
    将聚合后的结果写入 CSV 文件。
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="根据特定模式扫描设备信息，按描述聚合 ID 并输出 CSV。")
    parser.add_argument(
        "--workers", 
        type=int, 
        default=50, 
