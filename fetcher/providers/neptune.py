...
class NeptuneProvider(ProviderBase):
    ...
    async def fetch_device_status(self, station: Station, device_id: str, session: ClientSession) -> Tuple[Optional[Dict[str, Any]], Optional[Exception]]:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                async with session.post(
                    api_address, data={"areaId": 6, "devaddress": device_id}, timeout=TIMEOUT
                ) as response:
                    response.raise_for_status()
                    json_data = await response.json()
                    if json_data.get("success") is not True:
                        return None, ValueError(f"API failed for device {device_id}: {json_data.get('msg')}")
                    ...
                    # 找到了API，但没找到设备
                    return None, ValueError(f"Device {device_id} not found in API response. {json_data}")
    ...
    async def fetch_station_status(self, station: Station, session: ClientSession) -> Tuple[Optional[Dict[str, Any]], Optional[Exception]]:
