...
async def startup_event():
    ...
    try:
        ...
        if stations_data:
            ...
            logger.info(
                f"已加载站点信息文件，共 {station_count} 个站点，更新时间: {updated_at}"
            )
        else:
...
def aggregate_stations_by_id(stations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for station in stations:
        ...
        if station_id not in stations_by_id:
            # 第一个站点，直接使用
            stations_by_id[station_id] = {"station": station.copy(), "count": 1}
        else:
            # 后续相同 id 的站点，忽略
    ...
    for station_id, data in stations_by_id.items():
        ...
        if data["count"] > 1:
            ...
            logger.info(
                f"聚合站点 '{station_name}' (id={station_id[:8]}...): 保留了第一个站点，合并了 {data['count']} 个站点"
            )
...
async def get_status(
    ...
    try:
        ...
        if cached_data and cached_data.get("stations"):
            ...
            if provider:
                has_provider_data = any(
                    s.get("provider_id") == provider for s in stations
                )
        ...
        if use_cache:
            ...
            if provider:
                result["stations"] = [
                    s for s in result["stations"] if s.get("provider_id") == provider
                ]
                logger.info(
                    f"从缓存中过滤出 {len(result['stations'])} 个 {provider} 服务商的站点"
                )

            # 如果指定了 id，过滤出匹配的站点
            if id:
                result["stations"] = [
                    s for s in result["stations"] if s.get("id") == id
                ]
            ...
            if result is None:
                ...
                if cached_data:
                    ...
                    if not id:
                        # 聚合相同 id 的站点（只有在没有指定 id 时才聚合）
                        result["stations"] = aggregate_stations_by_id(
                            result["stations"]
                        )
                    return result
                else:
                    raise HTTPException(
                        status_code=500, detail="数据抓取失败且无缓存数据"
                    )
